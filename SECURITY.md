...
- Keep Peerovo to one replica. Its PeerJS peer registry, session capacity leases, and rate-limit counters are process-local.
...
PeerJS upgrades reject missing or duplicated `id`, `token`, or `key` parameters, mismatched public keys, invalid or expired tickets, and peer IDs that do not match the signed ticket. The service caps signaling payloads and active peers per session and per project. Admission uses short leases so abandoned or failed upgrades do not permanently consume capacity.

Periodic per-project usage summaries contain request and signaling counts plus active/peak peer counts. They do not include IP addresses, peer IDs, session IDs, project API keys, peer tickets, or TURN credentials. TURN media bandwidth is not visible to Peerovo and must be monitored on the coturn host.
